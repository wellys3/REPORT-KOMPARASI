...
*&      --> GIT_DATA
*&---------------------------------------------------------------------*
FORM f_display_data  USING  p_lit_data_komparasi_lv1_3 TYPE gtt_data_komparasi_lv1_3
                            p_lit_data_komparasi_lv4 TYPE gtt_data_komparasi_lv4.
  ...
  CASE s_zlevel-low.
    WHEN 1 OR 2 OR 3.
      lcl_falv ?= gcl_falv=>create( EXPORTING  i_subclass = cl_abap_classdescr=>describe_by_name( p_name = 'GCL_FALV' )
                                    CHANGING ct_table = p_lit_data_komparasi_lv1_3 ) .
    WHEN 4.
      lcl_falv ?= gcl_falv=>create( EXPORTING  i_subclass = cl_abap_classdescr=>describe_by_name( p_name = 'GCL_FALV' )
                                    CHANGING ct_table = p_lit_data_komparasi_lv4 ) .
  ENDCASE.
